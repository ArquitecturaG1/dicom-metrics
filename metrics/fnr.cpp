...
float fnr(int width, int height, int *img_reference, int *img_detected){
     ...
     * True positive -> reference=1 & detected=1 -> CASO REQUERIDO
    ...
    // Obtencion del numero de pixeles falsos negativos y verdaderos positivos
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            if (*((img_reference+i*height)+j) == 1 && *((img_detected+i*height)+j) == 1)
                true_positives++;
    ...
    fflush(stdout);
    // Obtencion de la tasa de falsos negativos
    return (float)false_negatives / (true_positives + false_negatives);
}
